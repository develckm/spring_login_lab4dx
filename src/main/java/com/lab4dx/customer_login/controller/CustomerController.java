...
public class CustomerController {
    ...
    //customer/orders.html 를 렌더링
    //@SessionAttribute(required = true) : 세션에서 객체를 가져오겠다.
    //=>꼭 로그인한 유저만 이페이지를 요청할 수 있다.
    //만약 로그인하지 않은 유저가 요청하면 400(클라이언트 잘못)에러를 발생시킨다.
    @GetMapping("/orders.do")
    public void orders(
            @SessionAttribute Customer customer,
            Model model) {
        Customer ordersCustomer=customerService.get(customer.getCustomer_id());
        model.addAttribute("customer",ordersCustomer);
    }
}
